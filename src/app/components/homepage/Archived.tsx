...
const FinishedProjects = ProjectsList.filter(
  (project) => !project.draft && project.archived
);
...
const Archived: FC<SectionProps> = ({ updateActiveSection }) => {
  ...
  useEffect(() => {
    if (isInView) {
      updateActiveSection(2);
    }
  }, [isInView]);

  return (
    <section ref={ref} id="archived">
      <h1 className="font-bold tracking-tight text-xl p-3 md:px-6">Archived</h1>
      <ul className="flex flex-col w-full group/Archived">
          ...
          return (
            <Link href={project.pageLink} key={index}>
              ...
              >
                ...
                {isHovered && (
                  <motion.div
                    layoutId="a_indicator"
                    transition={{
                      duration: 0.2,
                    }}
                    className={`absolute bg-zinc-100 inset-0 w-full h-full rounded-xl opacity-0 group-hover/Archived:opacity-100 transition-opacity duration-300 flex items-center text-xs text-zinc-600`}
                  ></motion.div>
                )}
              </li>
            </Link>
